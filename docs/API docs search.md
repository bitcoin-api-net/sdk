...
46. Создать репозиторий `apps/api/src/repositories/openapi.repository.ts` (+ `openapi.repository/types.ts`) — **in-memory** хранилище OpenAPI схемы. API: `save(schema: object)` (внутри сразу делает `writeToFile()`), `getSchema()`, `findOperation(method, path)`, `findOperationById(operationId)`, `listOperations()`, `writeToFile(): string`. Никакого `fs.readFileSync` в рантайме — источник правды для MCP это тот же объект, что вернул `app.swagger()`. `writeToFile()` инкапсулирует запись `apps/api/files/openapi.json` для билда web-client (Фаза 1) — путь резолвится от `import.meta.dirname`.
47. В `apps/api/src/app.ts` после `app.ready()`: `openApiRepository.save(app.swagger())` (сам пишет файл внутри). Один объект, два потребителя (MCP в памяти + web-client билд через файл) — никаких race conditions с диском.
48. Создать usecase-ы в `apps/api/src/usecases/docs/` (рядом с уже существующим `ask-ai.usecase.ts`):
    48.1. `docs-search.usecase.ts` — `DocsSearchUseCase`, инжектит `googleAiProvider`, `docChunkRepository`, `recipeChunkRepository`, `apiChunkRepository`. `execute({query, k?})`: `embed(query, 'RETRIEVAL_QUERY')` → параллельный `searchByVector` по трём репо (over-fetch `k * PER_SOURCE_MULTIPLIER`) → merge → rank-based similarity (`1/(rank+1)`) → top-`k`. Default `k=8`. Используется только AI-search'ем (Фаза 7), MCP-тулзы это не вызывают — MCP отдаёт листинг + fetch без векторного поиска.
    ...
    48.3. `recipe-search.usecase.ts` — `RecipeSearchUseCase`, инжектит `googleAiProvider`, `recipeChunkRepository`. `execute({operationId?, query?, language?, k?})`: ветка с `operationId` → `findByEndpoint` (+опц. лексический re-rank по `query`); ветка с `query` → `searchByVector` (over-fetch `k * 3` чтобы оставить место для пост-фильтра); пост-фильтр по `language` (case-insensitive). Default `k=10`. `RecipeChunk` тип получаем через `Awaited<ReturnType<...>>` чтобы не тащить prisma client напрямую. На MCP сейчас выставлена только ветка по `operationId` (через тулзу `get_recepies_for_endpoint`).
    48.4. `recipe-fetch.usecase.ts` — `RecipeFetchUseCase`, инжектит `recipeChunkRepository`. `execute({url})` → `findByUrl(url)` → склейка чанков рецепта в один markdown + метаданные (`title`, `description`, `language`, `difficulty`, `tags`, `runUrl`, `endpoints`). Аналог `docs-fetch`, но для рецептов.
49. Создать MCP server singleton в `apps/api/src/mcp/server.ts`:
    - `new McpServer({ name: 'bitcoin-api-docs', version: '0.0.1' })`;
    - вызвать `registerDocsListTool/registerDocsFetchTool/registerRecipeSearchTool/registerRecipeFetchTool/registerApiEndpointsListTool/registerApiEndpointTool` в нужном порядке;
    - функция `wrapToolHandlers(server)` — оборачивает каждый зарегистрированный handler через приватный `_registeredTools` (у `McpServer` нет публичного pre/post-хука; альтернатива — низкоуровневый `Server` с ручным `setRequestHandler`, но это потеря всей валидации zod + структурирования). Делает две вещи: логирует `{tool, durationMs, ok}` для аналитики и маппит `AppError` (например `NotFoundError`) в `{content: [{type:'text', text: 'CODE: message'}], isError: true}`, чтобы тулзы оставались one-liner-ом вокруг usecase-а.
50. Создать 6 файлов-обёрток в `apps/api/src/mcp/tools/` — каждый экспортирует `register*Tool(server)` с zod input schema и тонким хендлером. Возврат: `{content: [{type: 'text', text: JSON.stringify(...)}], structuredContent: {...}}` или `isError: true` при отсутствии данных. Список тулз:
    - `get_docs_list` (`get-docs-list.ts`) — без параметров, отдаёт все doc-чанки (url, anchor, title);
    - `get_doc` (`get-doc.ts`) — параметр `url`, отдаёт полный markdown страницы;
    - `get_recepies_for_endpoint` (`get-recepies-for-endpoint.ts`) — параметр `operationId`, отдаёт список рецептов под этот endpoint;
    - `get_recipe` (`get-recipe.ts`) — параметр `url`, отдаёт полный markdown рецепта + метаданные;
    - `get_api_endpoints_list` (`get-api-endpoints-list.ts`) — без параметров, отдаёт список всех endpoint-ов (`operationId/method/path/summary/description/tags`);
    - `api_endpoint` (`get-api-endpoint.ts`) — параметр `operationId`, отдаёт raw OpenAPI operation; на отсутствие кидает `NotFoundError` (его маппит `wrapToolHandlers`).
