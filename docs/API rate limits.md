...
WS connect ──► IP gate (counter, fixed) ──► auth (apiKey → userId) ──► concurrent gauge per (userId | ip, routeId) ──► accept / close(1008)
                                                                                      │
...
- **Дефолт = anonymous = authenticated-without-boost**. Один и тот же лимит и для анонимов (по IP), и для запросов с ключом без буста. Буст применяется только к конкретному `(userId, operationId)` и поднимает лимит сверх дефолта.
...
Один и тот же дефолт действует и для анонимов (по IP), и для запросов с ключом без буста. Буст применяется только к конкретному `(userId, operationId)` и поднимает лимит **сверх** дефолта (т.е. итог = `max(default, boost.rateLimit)`; на практике `boost > default`). Все активные ключи юзера автоматически подхватывают буст — лимит и счётчик общие.
...
1. Если `userId` есть:
   1. `boostCache.get(userId, routeId)` → попадание → готово.
   2. Иначе: `findFirst(Boost where userId, routeId, (expiresAt IS NULL OR > now))` → если есть, берём `boost.rateLimit`; иначе берём `defaultLimit`.
   3. Кладём результат в `boostCache` с TTL 60s (кешируем и дефолт, чтобы не дёргать БД на каждый запрос).
...
Redis-стор встроен в плагин (опция `redis: <ioredis instance>`). Либа сама делает `INCR <prefix>:<key>` + `EXPIRE 60` при первом инкременте. Ключ формируется через `keyGenerator: (req) => req.userId ? 'u:' + req.userId : 'ip:' + req.ip`. Изоляция счётчиков per route — через встроенный `groupId` (передаём `routeId`, под капотом склеивается с key).
...
- **`apiKeyCache`**: `token` → `{userId, isActive}`. TTL 60s. Инвалидация через канал `rl:invalidate-key` (payload: `{token}`) — при rotate/delete ключа.
...
5. `shared/src/repositories/api-key.repository.ts` — `findByToken(token)` (hot path, возвращает `{id, userId, isActive, ...}`), `findById(id)`, `listByUserId(userId)`, `create({userId, name})` (генерит токен, пишет в `ApiKey`, возвращает целиком), `touchLastUsed(id)`, `setActive(id, isActive)`, `delete(id)`.
6. `shared/src/repositories/boost.repository.ts` — `findActive(userId, routeId)` (фильтрует по `expiresAt`), `upsert({userId, routeId, rateLimit, expiresAt, stripeSubscriptionItemId, stripePriceId})`, `deleteByStripeItemId(itemId)`, `listByUserId(userId)`.
...
8. `apps/api/src/services/api-key-cache.service.ts`:
   ...
   - `resolve(token): Promise<{userId: string, apiKeyId: string, isActive: boolean} | undefined>` — LRU → `apiKeyRepository.findByToken(token)` → результат в LRU с TTL 60s. Неактивные ключи кешируем как `isActive=false` (чтобы не бить БД на флуд невалидных токенов), но TTL для них короче (напр. 10s) — на случай reactivation.
...
9. `apps/api/src/services/rate-limit-config.service.ts`:
   ...
   - `resolve(userId: string | undefined, routeId: string, defaultLimit: number): Promise<number>` — без `userId` сразу вернуть `defaultLimit`. С `userId`: LRU → `boostRepository.findActive(userId, routeId)` → если есть, вернуть `boost.rateLimit`, иначе `defaultLimit`. Положить результат в LRU с TTL 60s.
...
11. `apps/api/src/plugins/rate-limit.ts` (через `fp(...)`):
    - **`onRequest` auth-хук (до `@fastify/rate-limit`)**: парсит `X-Api-Key`:
      ...
      - есть заголовок → `apiKeyCacheService.resolve(token)`. Если `undefined` или `isActive=false` → отвечаем **401** и прерываем цепочку;
      - иначе: `req.userId = userId`, `req.apiKeyId = apiKey.id`, `req.apiKey = token`.
    - регистрирует `@fastify/rate-limit` глобально с:
      ...
      - `max: async (req) => await rateLimitConfigService.resolve(req.userId, req.routeOptions.schema.operationId, req.routeOptions.schema['x-default-rate-limit'])`;
      - `errorResponseBuilder` — формат `AppError`-style для consistency;
    - регистрируется ПОСЛЕ `jwtAuthPlugin`.
12. Расширить `FastifyRequest` полями `userId?: string`, `apiKey?: string`, `apiKeyId?: string` через `declare module 'fastify'`. (`userId` может уже быть объявлен JWT-плагином — проверить на коллизию при реализации: если да, договариваемся о едином поле и используем его.)
...
13. `apps/api/src/plugins/rate-limit-ws.ts`:
    ...
    - **rate-counter** (новые connections per minute, per `(userId | ip, operationId)`) — через тот же `@fastify/rate-limit` (явный вызов `fastify.rateLimit({ max, timeWindow, groupId })` в preHandler с `groupId = operationId`). Лимит резолвится через `rateLimitConfigService.resolve(req.userId, operationId, connectRateDefault)` точно как для REST;
    - **gauge** (concurrent connections, per `(userId | ip, operationId)`) — ручной INCR/DECR с TTL safety-net. Ключ `rl:ws:gauge:{u:userId | ip:ip}:{operationId}`. Лимит = `concurrentDefault`. Бусты на concurrent в MVP **не делаем** (`Boost.rateLimit` относится только к rate);
...
18. `apps/api/src/usecases/api-keys/rotate-api-key.usecase.ts` — перегенерит токен, обновляет `ApiKey`. `Boost` **не трогаем** (они привязаны к `userId`). После — `apiKeyCacheService.invalidate(oldToken)` и `invalidate(newToken)` (второе — на всякий, чтоб вытеснить возможный stale negative-cache).
19. `apps/api/src/usecases/api-keys/delete-api-key.usecase.ts` — удаляет `ApiKey` + `apiKeyCacheService.invalidate(token)`. `Boost` юзера не трогаем — они применятся к его остальным ключам.
20. `apps/api/src/usecases/boosts/apply-boost.usecase.ts` — апсёрт в `Boost` per `(userId, routeId)` + `rateLimitConfigService.invalidateUser(userId)`. Вызывается из webhook-handler'а Stripe.
21. Соответствующие REST роуты:
...
23. Логи warn при 429 (с `userId` / `apiKeyId` / `ip` / `routeId`) — через `errorResponseBuilder` + `request.log.warn`. Плейн-токен в логи **не пишем** (хоть он и хранится в БД — в логах он лишний шум и лишняя поверхность утечки). Логируем `userId` (связь с бустом при дебаге) и `apiKeyId` (какой именно ключ юзера сейчас упирается в лимит).
