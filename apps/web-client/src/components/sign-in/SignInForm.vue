...
async function handleSignIn() {
  ...
  try {
    const res = await fetch(`${import.meta.env.PUBLIC_API_URL}/v1/auth/login`, {
      method: 'POST',
...
<template>
  <form @submit.prevent="handleSignIn" class="space-y-6">
    <div>
      ...
      <input v-model="email" type="email" class="w-full h-11 px-3 bg-white border border-[#dee1e6] rounded-xl text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-[#2563eb] outline-none transition-all" />
    </div>
    ...
    <div class="relative">
      ...
      <div class="relative">
        ...
        <button type="button" @click="showPassword = !showPassword" class="absolute right-3 top-1/2 -translate-y-1/2 text-[#8F96A3] hover:text-[#323843]">
    ...
    <div class="flex items-center gap-2">
      <div class="relative flex items-center">
        <input v-model="rememberMe" type="checkbox" id="remember" class="custom-checkbox" />
      </div>
    ...
    <button type="submit" class="w-full h-11 bg-[#2563EB] text-white rounded-xl text-sm font-medium shadow-sm hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-70" :disabled="currentStatus === 'Loading'">
